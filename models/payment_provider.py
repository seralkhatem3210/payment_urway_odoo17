...
class PaymentProviderUrway(models.Model):
    ...
    def _get_default_payment_method_id(self):
        ...
        basic_payment_method = self.env['payment.method'].search([
            ('code','=','urway'),
        ], limit=1)
        if not basic_payment_method:

            try:
                create_basic_payment_method = self.env['payment.method'].create({
                    'active': True,
                    'code': 'urway',
                    'is_primary': True,
                    'name': 'URWAY',
                ...
                _logger.error(f"Error creating payment method: {e}")
    ...
    def _update_payment_method_lines(self):
        ...
        # payment_methods = self.env['account.payment.method'].search([('provider_ids', 'in', self.ids),('code','=',self.code)])
