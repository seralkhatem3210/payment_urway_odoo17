...
class PaymentProviderUrway(models.Model):
    ...
    # payment_account_id = fields.Many2one('account.account', string="Payment Account", required=True)
    ...
    def _get_default_payment_method_id(self):
        """Override to ensure URWAY payment method and method line exist."""
        # self.ensure_one()
        payment_methods = self.env['payment.method']
        for record in payment_methods:
            record.provider_ids = self.id
        ...
        if self.code == 'urway':
            ...
            self.payment_method_ids = payment_method
        self.payment_method_ids = payment_method
        ...
        if self.code == 'urway':
            ...
            # Ensure journal_id and account exist
            if not self.journal_id or not self.payment_account_id:
                raise ValidationError(_("Journal ID and Payment Account ID are required for the URWAY provider."))
    ...
    #     if self.journal_id:
    #         payment_method_line.journal_id = self.journal_id
    #     else:
    #         _logger.warning("Journal ID is missing for provider %s", self.id)
    ...
    def _update_payment_method_lines(self):
        """Update payment method lines for the provider."""
        _logger.info("Updating payment method lines for provider IDs: %s", self.ids)
        ...
        if payment_method_lines:
            ...
            if not payment_method_line:
                ...
                payment_method_line = self.env['account.payment.method.line'].create({
                    ...
                    'sequence': 10,  # Default sequence
                    'payment_provider_id': self.id,  # Link to the URWAY provider
                    # 'payment_account_id': self.journal_id.payment_account_id.id,  # Assign payment account
                    # 'state': 'test',  # Set the state, could be dynamic based on environment
                })
        ...
        for payment_method_line in payment_method_lines:
            if self.journal_id:
                payment_method_line.journal_id = self.journal_id
                _logger.info("Updated journal_id for payment method line: %s", payment_method_line.id)
            else:
                _logger.warning("No journal_id defined for provider %s", self.id)
...
class PaymentToken(models.Model):
    ...
    def _get_available_tokens(self, *args, **kwargs):
        # Check if the parent method exists and call it; otherwise, provide fallback logic
        if hasattr(super(PaymentToken, self), '_get_available_tokens'):
            return super(PaymentToken, self)._get_available_tokens(*args, **kwargs)
        else:
            _logger.warning("Parent method '_get_available_tokens' not found. Using fallback logic.")
